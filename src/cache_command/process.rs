...
use crate::{db::DbOp, prelude::colors::*};

use super::{
    cache::*,
    clear_terminal,
    os_command::exec_command,
    parser::{parse_command, CacheCommand},
};

const CACHE_COMMAND_DOC: &[(&[&str], &str)] = CacheCommand::doc();
const BACKUP_FILE_NAME: &str = "karsherdb.json";

fn check_reserved_command(aliases: &[&str]) -> bool {
    CACHE_COMMAND_DOC
        .iter()
        .flat_map(|c| c.0.iter())
    ...
    match parse_command(line) {
        Ok((_, command)) => match command {
            CacheCommand::Put { aliases, value } => {
                if check_reserved_command(&aliases) {
