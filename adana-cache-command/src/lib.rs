...
impl CacheCommand<'_> {
    pub const fn doc() -> &'static [(&'static [&'static str], &'static str)] {
        ...
        &[
            (
                &[PUT],
                "Put a new value to current namespace. can have multiple aliases with option '-a'. e.g `put -a drc -a drcomp docker-compose`",
            ),
            (
                &[DESCRIBE, DESCRIBE_ALT],
                "List values within the current namespace. Optional parameter for searching e.g `ds ssh`",
            ),
            (&[LIST_CACHE, LIST_CACHE_ALT], "List available namespaces."),
            (&[CURR_CACHE, CURR_CACHE_ALT], "Print current namespace."),
            (
                &[BACKUP, BACKUP_ALT],
                "Backup the database of namespaces to the current directory",
            ),
            ...
            (
                &[DEL_CACHE, DEL_CACHE_ALT],
                "Delete namespace or clear current namespace values.",
            ),
            (
                &[MERGE_CACHE, MERGE_CACHE_ALT],
                "Merge current with a given namespace",
            ),
            (&[DEL, DEL_ALT], "Remove value from namespace. e.g `del drc`"),
            (&[GET], "Get value from namespace.  e.g `get drc`"),
            (
                &[CLIPPY, CLIPPY_ALT],
                "Get value from namespace and copy it to clipboard. e.g `clip drc`",
            ),
            (
                &[EXEC],
                "Run a value from the namespace as an OS command. e.g `exec drc`",
            ),
            (&[CD], "Navigate to a directory"),
            (
                &[USE],
                "Switch to another namespace. default ns is DEFAULT. e.g `use linux`",
            ),
            (
                &[DUMP],
                "Dump namespace(s) as json. Take an optional parameter, the namespace name. e.g `dump linux`",
            ),
            (&[CLEAR, CLEAR_ALT], "Clear the terminal."),
            (
                &[PRINT_SCRIPT_CONTEXT, PRINT_SCRIPT_CONTEXT_ALT],
                "Print script context",
            ),
            (
                &[STORE_SCRIPT_CONTEXT],
                "Store script context (name optional), e.g store_script_ctx 12022023",
            ),
            (
                &[LOAD_SCRIPT_CONTEXT],
                "Load script context (name optional), e.g load_script_ctx 12022023",
            ),
